...
class SinglyLinkedList {
	...
	public void insertBetween(int data, int index) {
		if (index > size) {
			...
			Node  newest = new Node  (data, temp1);
			temp.setNext(newest);
			size++;
		}
	}
	...
	public int removeFirst() { // removes and returns the first element
		if (isEmpty()) return -1; // nothing to remove
    ...
    private boolean checkPalindrome(Node head) 
    {
        if(head == null)
            return true;
        boolean x =checkPalindrome(head.getNext());
        if(x == false)
            return false;
        boolean y=(temp.getElement() == head.getElement());
